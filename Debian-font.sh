...
then
    ...
    then
        ...
        case "$choice" in 
          y|Y ) if [ "$wgetexists" != "/usr/bin/wget" -o $fontcacheexists != "/usr/bin/fc-cache" ]; then sudo apt install fontconfig wget --noconfirm --needed;fi;;
...
if [ ! -z "$1" ]
  then
    ...
    then
        if [ "$wgetexists" != "/usr/bin/wget" -o $fontcacheexists != "/usr/bin/fc-cache" ]
        then
            sudo apt install fontconfig wget --noconfirm --needed;
        fi
    else
